...
namespace DijkstraTest2
{
    ...
    class SetUpGraph
    {
        ...
        public void FindPaths(string source, string last,TextBox tbKM,TextBox tbLiter, TextBox tbCost, TextBox tbPath, TextBox tbTime)
        {
            ...
            int v = Convert.ToInt32(last);
            {
                if (v != s)
                {
                    ...
                    {
                        FindPath(s, v,tbKM, tbLiter, tbCost, tbPath, tbTime);
                    }
                }
            }
        }

        public void FindPath(int s, int v, TextBox tbKM, TextBox tbLiter, TextBox tbCost , TextBox tbPath, TextBox tbTime)
        {
            int i, u;
            int[] path = new int[n];
            int km = 0;
